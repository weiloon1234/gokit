...
// The CountryLocationFunc type is an adapter to allow the use of ordinary
// function as CountryLocation mutator.
type CountryLocationFunc func(context.Context, *ent.CountryLocationMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f CountryLocationFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.CountryLocationMutation); ok {
		return f(ctx, mv)
	}
